...
padronizar_bairros <- function(bairros) {
  ...
  bairros_padrao <- stringr::str_replace_all(
    ...
    c(
      ...
      "\\bSTA\\b\\.?" = "SANTA",
      "\\bSRA\\b\\.?" = "SENHORA",
      "\\b(N(OS|SS?A?)?\\.? SENHORA|(NOSSA|NSA\\.?) (SR?|SEN(H(OR)?)?))\\b\\.?" = "NOSSA SENHORA",
      "\\b(NS?\\.? S(R|ENH?)?\\.?( DE?)?|NOSSA SENHORA) (FAT.*|LO?UR.*|SANTANA|GUADALUPE)\\b" = "NOSSA SENHORA DE \\4",
      ...
      "\\b(NS?\\.? S(R|ENH?)?\\.?( D(A|E)?)?|NOSSA SENHORA D(A|E)) (APARECIDA|AUXILIADORA|MEDIANEIRA|CONSOLADORA)\\b" = "NOSSA SENHORA \\6",
      "\\b(NS?\\.? S(R|ENH?)?\\.?( D(OS?)?)?|NOSSA SENHORA) (NAVEGANTES)\\b" = "NOSSA SENHORA DOS \\5",
      "\\b(NS?\\.? S(R|ENH?)?\\.?( DO?)?|NOSSA SENHORA) (CARMO|LIVRAMENTO|RETIRO|SION|ROSARIO|PILAR|ROCIO|CAMINHO|DESTERRO|BOM CONSELHO|AMPARO)\\b" = "NOSSA SENHORA DO \\4",
